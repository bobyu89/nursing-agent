...
function shiftInterval(dateStr, shiftCode) {
  const type = SHIFT_TYPES[shiftCode];
...
/** 本週已排定工時（小時） */
function weeklyHours(staffId) {
  return shiftsOf(staffId)
    .filter((s) => WEEK_DATES.includes(s.date))
    .reduce((sum, s) => sum + SHIFT_TYPES[s.shift].hours, 0);
}

/** 本週各班別次數，用於 S3 班別型態相符 */
function shiftMix(staffId) {
  const mix = { D: 0, E: 0, N: 0, total: 0 };
  shiftsOf(staffId)
    .filter((s) => WEEK_DATES.includes(s.date))
    .forEach((s) => { mix[s.shift] += 1; mix.total += 1; });
  return mix;
}
...
function checkHardConstraints(staff, gap) {
  ...
  if (getHardRule('H1').enabled) {
    const need = ROLE_LEVELS[gap.requiredRole];
    const have = ROLE_LEVELS[staff.role];
    if (have < need) {
    ...
    gap.requiredCerts.forEach((c) => {
      ...
      if (!expiry) {
        violations.push({ code: 'H1', detail: `未具備必要資格：${CERTS[c]}` });
      } else if (expiry < gap.date) {
        violations.push({ code: 'H1', detail: `${CERTS[c]} 證照已於 ${expiry} 到期` });
      }
    });
  }
  ...
  if (getHardRule('H2').enabled) {
    ...
    if (same) {
      violations.push({
        code: 'H2',
        detail: `當日已排 ${SHIFT_TYPES[same.shift].name}（${SHIFT_TYPES[same.shift].start}–${SHIFT_TYPES[same.shift].end}）`,
      });
    }
  }
  ...
  if (getHardRule('H4').enabled) {
    ...
    if (rest.against && rest.hours < minRest) {
      ...
      const label = `${shortDate(s.date)} ${SHIFT_TYPES[s.shift].name}`;
      violations.push({
        code: 'H4',
        detail: rest.hours < 0
          ? `與 ${label} 時間重疊`
  ...
  if (getHardRule('H6').enabled) {
    ...
    const projected = weeklyHours(staff.id) + SHIFT_TYPES[gap.shift].hours;
    if (projected > cap) {
      violations.push({ code: 'H6', detail: `替補後週工時 ${projected} 小時，超過絕對上限 ${cap} 小時` });
    }
  }
...
function scoreCandidate(staff, gap) {
  ...
  const base = weeklyHours(staff.id);
  const projected = base + SHIFT_TYPES[gap.shift].hours;
  ...
  breakdown.push({
    ...
    evidence: `本週已排 ${base} 小時，替補後 ${projected} 小時（軟性上限 ${softCap} 小時）`,
  });

  // S3 班別型態相符
  const mix = shiftMix(staff.id);
  const s3Ratio = mix.total === 0 ? 0.5 : mix[gap.shift] / mix.total;
  breakdown.push({
    code: 'S3', name: '班別型態相符', weight: getSoftWeight('S3'),
    ratio: s3Ratio, points: round1(s3Ratio * getSoftWeight('S3')),
    evidence: mix.total === 0
      ? '本週無既有班別，以中間值計'
      : `本週 ${mix.total} 個班中有 ${mix[gap.shift]} 個${SHIFT_TYPES[gap.shift].name}`,
  });
  ...
  if (staff.willingShifts === null) {
    ...
    s4Ratio = 1; s4Evidence = `已自填願意支援${SHIFT_TYPES[gap.shift].name}`;
  } else {
    s4Ratio = 0;
    s4Evidence = `自填願意支援 ${staff.willingShifts.map((c) => SHIFT_TYPES[c].name).join('、')}，不含${SHIFT_TYPES[gap.shift].name}`;
  }
  breakdown.push({
  ...
  if (staff.unit === gap.unit) {
    s5Ratio = 1; s5Evidence = `本單位人員（${UNITS[gap.unit]}）`;
  } else if (staff.familiarUnits.includes(gap.unit)) {
    s5Ratio = 0.6; s5Evidence = `隸屬 ${UNITS[staff.unit]}，曾支援過 ${UNITS[gap.unit]}`;
  } else {
    s5Ratio = 0; s5Evidence = `隸屬 ${UNITS[staff.unit]}，無 ${UNITS[gap.unit]} 支援紀錄`;
  }
  breakdown.push({
...
function collectFlags(staff, gap, score) {
  ...
  if (score.projected > softCap) {
    flags.push({
      ...
      text: `替補後週工時 ${score.projected} 小時，超過軟性上限 ${softCap} 小時，需單位主管額外核准`,
    });
  }
  const days = consecutiveDaysWithGap(staff.id, gap.date);
  if (days >= 5) {
  ...
  if (staff.unit !== gap.unit) {
    flags.push({
      ...
      text: `跨單位支援（${UNITS[staff.unit]} → ${UNITS[gap.unit]}），需加強交班`,
    });
  }
...
/**
 * 對單一缺班事件評估所有人員。
 * @returns {{candidates: Array, excluded: Array, evaluatedAt: string}}
 */
...
function relaxationAnalysis(gap) {
  ...
  const options = [];
  RULE_REGISTRY.hard.forEach((rule) => {
...
function applyReplacement(gap, staffId) {
  SHIFTS.push({ staffId, date: gap.date, shift: gap.shift, unit: gap.unit, isReplacement: true });
...
function scheduleDelta(gap, staffId) {
  const dayShifts = SHIFTS.filter((s) => s.date === gap.date && s.unit === gap.unit);
  return {
    date: gap.date,
    unit: gap.unit,
