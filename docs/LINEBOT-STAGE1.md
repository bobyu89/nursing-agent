...
Stage 0 的六格選單全是護理長視角。Stage 1 有兩種人，LINE 支援按使用者綁定不同選單
（`POST /v2/bot/user/{userId}/richmenu/{richMenuId}`），而**綁定成功那一刻就知道角色**——
宿主在 `bindFlow` 回傳後依 `role` 掛對應選單，不做共用大選單。

| 角色 | 格子 | 上線 Phase |
|---|---|---|
| 未綁定（預設選單） | 單格「如何綁定」 | 選配；未綁定者按任何鍵本來就會收到綁定說明 |
| 護理師 | 通報缺班／我的班表／預班／我的替班邀請／使用說明 | 通報、邀請＝Phase 1；預班＝Phase 2 |
| 護理長 | 儀表板／待核准／換班／調度／負荷／使用說明 | 待核准＝Phase 1 |

`richmenu.ps1` 現有的建立與掛載流程沿用，改成建三份、由宿主依角色掛。
...
| **1 替班迴路** | §4 全部：通報→核准 Flex→逐一問→逾時→寫回→回報；護理師／護理長兩份圖文選單（§2.4） | **當天就能實測**的完整迴路；landing page「我們開始在那通電話」成真 | 2–3 週 |
| **2 預班迴路** | §3 全部：開啟→文字預假→催繳→截止→生成→審核→公告；護理師選單加「預班」 | 月度預班；要跑過一個月才驗得完 | 2 週 |
