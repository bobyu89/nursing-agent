...
- **寫回（Phase 3d ✅）**：登入模式下護理長的每一次班表變更（格子編輯、匯入、套用生成草稿、換班核准、主管確認寫回）
  去抖動 800ms 後把範圍內的整份班表 `POST /api/shifts`；Worker 逐筆白名單驗證、算差異（key＝代號|日期|班別，單位或來源不同＝刪＋加）、
  以 `baseVersion`（範圍內班次的雜湊）做樂觀鎖——雲端在你載入後被動過（另一位護理長、機器人的替班寫回）就回 409 不覆蓋、
  平台提示重新載入——批次寫入（來源 manual／substitution／swap）、留痕 `roster.writeback`（單位、加減筆數、樣本）。
  護理師唯讀、督導檢視模式：Worker 403，平台不會送。本機 localStorage 在登入模式下不載不寫。
...
| **3 收斂** ✅ 2026-09-20／3d 2026-09-21 | **3a** 生成需求可設定（`設定需求 D2 E1 N1`，存 setting `req.<unit>`，截止生成即用）；**3b** 平台以 LINE 身分登入——`平台` 回本人專屬簽章連結（10 分鐘）→ 平台換 session（12 小時）→ `GET /api/snapshot` 讀 D1（head／staff 只拿本單位；exec 全院）、視角鎖定權責層、本機舊編輯不疊上；**3c** 預假日曆頁 `prebook.html`（公告／催繳／我的預假附本人連結，`POST /api/prebook` 轉成同一句「預假 …」走 prebookFlow，同一套驗證同一筆留痕）；aws 宿主明寫不支援；**3d** 平台編輯寫回 D1（整份送回、宿主算差異、樂觀鎖 409、留痕差異；護理長限定） | 平台與 bot 同一份班表（讀＋寫） | 1–2 週 |
